...
async function getLastPrice(asset) {
    ...
    return cache.rows[0]?.last_price || null;
}

// ==========================
// SIGNAL GENERATION (REAL MARKET) - used by auto scanner
// ==========================
...
// ==========================
// MANUAL SIGNAL GENERATION (for frontend trial button)
// ==========================
...
app.post('/api/trades/generate-signal', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!currentPrice) return res.status(500).json({ error: 'Price fetch failed' });

        const lastPrice = await getLastPrice(assetSymbol);
        if (!lastPrice) return res.status(400).json({ error: 'No cached price yet, wait a moment' });

        const movement = ((currentPrice - lastPrice) / lastPrice) * 100;
        const volatility = Math.abs(movement);
        if (volatility < 0.03) return res.json({ success: true, message: 'No significant movement', price: currentPrice });

        const direction = movement > 0 ? 'BUY' : 'SELL';
        const confidence = calculateConfidence(movement, volatility);
        if (confidence < 75) return res.json({ success: true, message: 'Signal confidence too low', confidence });
...
// ==========================
// TEMPORARY: DELETE USER BY EMAIL OR PHONE (remove after use)
// ==========================
...
app.get('/api/admin/delete-user-by-phone', async (req, res) => {
    ...
    try {
        ...
        res.send(`✅ User with phone ${phone} deleted successfully.`);
    } catch (err) {
        res.status(500).send('Error: ' + err.message);
    }
});
